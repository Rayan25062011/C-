...
class Parser:
    ...
    def ParseEOL(self, code: str) -> str:
        ...
        for line in code.splitlines():
            skipLine = False
            for token in ("fn", "void", "while", "for", "if", "else", "elif", "with", "from", "do", "catch"):
    ...
    def ParseBraces(self, code: str) -> str:
        ...
        for line in splitLines:
            ...
            if "public class" in line:
                if not self.IsInString("public class", line):
                    line = "\n"+" ".join(line.split())
            if "fn" in line:
    ...
    def ParseFunctions(self, code: str) -> str:
        ...
        for line in code.splitlines():
            if "println" in line and not self.IsInString("println", line):
                code = code.replace(line, line.replace("println", "print"))
        for line in code.splitlines():
        ...
        for line in code.splitlines():
            if "do" in line and not self.IsInString("do", line):
                code = code.replace(line, line.replace("do", "try"))
        for line in code.splitlines():
            if "catch" in line and not self.IsInString("catch", line):
                code = code.replace(line, line.replace("catch", "except"))
        for line in code.splitlines():
    ...
    def CleanCode(self, code : str) -> str:
        ...
        if Exception in code.splitlines():
