...
class Parser:
    ...
    def ParseFunctions(self, code: str) -> str:
        ...
        for line in code.splitlines():
            if "#define" in line and not self.IsInString("#define", line):
                code = code.replace(line, line.translate( { "#define": None } ))

        for line in code.splitlines():
            if "#static" in line and not self.IsInString("#static", line):
                code = code.replace(line, line.translate( { "#static": "global" } ))
