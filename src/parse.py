...
class Parser:
    ...
    def ParseEOL(self, code: str) -> str:
        ...
        for line in code.splitlines():
            skipLine = False
            for token in ("function", "while", "for", "if", "else", "else if", "with", "from"):
    ...
    def ParseFunctions(self, code: str) -> str:
        ...
        for line in code.splitlines():
            if "scanln" in line and not self.IsInString("scanln", line):
                code = code.replace(line, line.replace("scanln", "input"))
        for line in code.splitlines():
            if "::" in line and not self.IsInString("::", line):
                code = code.replace(line, line.replace("::", "."))
        for line in code.splitlines():
            if "#static" in line and not self.IsInString("#static", line):
                code = code.replace(line, line.replace("#static", "global"))
        for line in code.splitlines():
            if "#quit" in line and not self.IsInString("#quit", line):
                code = code.replace(line, line.replace("#quit", "exit()"))
        for line in code.splitlines():
            if "#free" in line and not self.IsInString("#free", line):
                code = code.replace(line, line.replace("#free", "del"))
        for line in code.splitlines():
            if "#setspace game" in line and not self.IsInString("#setspace game", line):
                code = code.replace(line, line.replace("#setspace pygame", "import pygame"))
        for line in code.splitlines():
