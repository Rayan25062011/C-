...
class Parser:
    ...
    def ParseInclude(self, code: str) -> str:
        ...
        for line in code.splitlines():
            words = line.split()
            for wordNo, word in enumerate(words):
                if word == "require" and not self.IsInString(word, line):
    ...
    def ParseEOL(self, code: str) -> str:
        ...
        for line in code.splitlines():
            skipLine = False
            for token in ("fn", "void", "while", "for", "if", "else", "elif", "with", "from", "try", "except"):
            ...
            for token in ("return", "scan", "delete"):
                if token in line and not self.IsInString(token, line):
                    skipLine = False
        ...
        for line in splitLines:
            ...
            if "class" in line:
                if not self.IsInString("class", line):
                    line = "\n"+" ".join(line.split())
            if "fn" in line:
                if line.partition("fn")[0].count("\"") != 0 and line.partition("function")[0].count("\"") % 2 == 0:
                    words = line.split()
                    for wordNo, word in enumerate(words):
                        if word == "fn":
                            speechCount = line.partition("fn")[2].count("\"")
                            otherCount = line.partition("fn")[2].count("'")
    ...
    def ParseFunctions(self, code: str) -> str:
        code = code
        for line in code.splitlines():
            if "fn" in line and not self.IsInString("fn", line):
                code = code.replace(line, line.replace("fn", "def"))
        for line in code.splitlines():
            if "public void" in line and not self.IsInString("public void", line):
                code = code.replace(line, line.replace("public void", "def"))
        for line in code.splitlines():
            if "void" in line and not self.IsInString("void", line):
                code = code.replace(line, line.replace("void", "def"))
        for line in code.splitlines():
            if "private void" in line and not self.IsInString("private void", line):
                code = code.replace(line, line.replace("private void", "def"))
        for line in code.splitlines():
            if "dump" in line and not self.IsInString("dump", line):
                code = code.replace(line, line.replace("dump", "print"))
        for line in code.splitlines():
            if "delete" in line and not self.IsInString("delete", line):
                code = code.replace(line, line.replace("delete", "del"))
        for line in code.splitlines():
            if "scan" in line and not self.IsInString("scan", line):
                code = code.replace(line, line.replace("scan", "input"))
        for line in code.splitlines():
    ...
    def CleanCode(self, code : str) -> str:
        ...
        if error in code.splitlines():
            print(f"IdentationError line: {line}")
        splitLines = code.splitlines()
